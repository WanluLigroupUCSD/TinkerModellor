...
class Tinker2PDB(TinkerSystem):
    ...
    def _pdb_connectivity(self, contents:List[str]) -> List[str]:
        """
        Add the connectivity information to the pdb file
        Args:
            contents (List[str]): The contents of the pdb file
        Returns:
            List[str]: The contents of the pdb file with connectivity information
        """
        for i in range(self.AtomNums):
            for j in self.Bonds[i]:
                if j > i+1:
                    contents.append(f"CONECT{i+1:>5d}{j+1:>5d}")

        return contents

    def _ssbond(self, contents: List[str]) -> List[str]:
        """
        Add the SSBOND information to the pdb file
        Args:
            contents (List[str]): The contents of the pdb file
        Returns:
            List[str]: The contents of the pdb file with SSBOND information
        """
        count = 1
        processed_pairs = []  # Step 1: Initialize the list to keep track of processed atom pairs
        for i in range(self.AtomNums):
            if self.AtomTypesStr[i][0] == 'S':
                for j in self.Bonds[i]:
                    if self.AtomTypesStr[j-1][0] == 'S':
                        pair = (min(i+1, j), max(i+1, j))  # Ensure the pair is ordered to avoid duplicates in reverse order
                        if pair not in processed_pairs:  # Step 4: Check if the pair has been processed
                            contents.append(f"SSBOND {count:>3d} Atom {pair[0]:>4d} - Atom {pair[1]:>4d}")
                            count += 1
                            processed_pairs.append(pair)  # Mark this pair as processed
        return contents

    def __str__(self) -> str:
        contents = []
        contents.append("HEADER Generated By TinkerModellor")
        self._ssbond(contents)
        # atoms
        for i in range(self.AtomNums):
            #print(i)
            ...
            contents.append(
                f"ATOM  {i + 1:>5d} {self.AtomTypesStr[i]:<4} "
                ...
                f"1.00 99.99           {self.AtomTypesStrPDB[i]:>2}"
            )
